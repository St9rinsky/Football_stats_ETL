...
def delete_fixture(cursor, match_id):
    cursor.execute(
        "DELETE FROM silver.fixtures WHERE match_id = %s",
        (match_id,)
    )
    ...
    try:
        for match in data["matches"]:
            ...
            if status == "FINISHED":
                ...
                delete_fixture(cursor, match.get("id"))
                loaded_results += 1

        connection.commit()
        print(f"Loaded {loaded_results} finished matches into silver.results")
